...
import { DashboardStats, RecentItem } from '../generated/prisma/index.js';
...
const getDashboardData = async (userId: number) => {
    ...
    const transformedUser = {
        ...
        name: user.name || '',
        email: user.email,
        avatar: null as string | null, // Not in schema yet
        initials: user.name
            ? user.name
                  .split(' ')
                  .map(n => n[0])
 ...
 */
const getRecentItems = async (userId: number, limit: number = 10): Promise<RecentItem[]> => {
    const user = await prisma.user.findUnique({
        where: { id: userId }
    });
 ...
 * @returns {Promise<Object>}
 */
const getSkillsCount = async (userId: number) => {
    const user = await prisma.user.findUnique({
        where: { id: userId },
        select: {
            dashboardStats: {
 ...
 * @returns {Promise<DashboardStats>}
 */
const createOrUpdateDashboardStats = async (
    userId: number,
    statsData: {
        skillsCount?: number;
        totalApps?: number;
